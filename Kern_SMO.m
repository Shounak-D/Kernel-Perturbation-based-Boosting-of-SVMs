function [alphas, offset] = Kern_SMO(data, targetLabels, boxConstraints, P, kernel, smoOptions)
%SEQMINOPT Sequential Minimal Optimization for SVM with a given Kernel matrix 
%by Shounak Datta, JRF, ISI Kolkata. Version - 17th July 2016
%
%   [ALPHA OFFSET] = KERN_SMO(TRAINING,Y,BOXC,PROB,KERNEL,SMOOPTIONS)
...
%   KERNEL is the kernel matrix used. SMOOPTIONS is optional struct 
...
[alphas, offset] = SMO(data, targetLabels, ...
    boxConstraints, P, kernel,  smoOptions);
...
function [alphas, offset] = SMO(data, targetLabels, ...
    boxConstraints, P, kernelMat, smoOptions)
...
for itCount = 1 : smoOptions.MaxIter
    ...
    idx1 = tmp(idx1);
    if isempty(idx1)
        idx1 = randi(length(targetLabels));
        val1 = targetLabels(idx1) .* objGrad(idx1);
    end
    ...
    if isempty(idx2)
        idx2 = randi(length(targetLabels));
        while idx2==idx1
            idx2 = randi(length(targetLabels));
        end
...
% -------------------------------------
    ...
    function [val2, idx2] = getMaxGain(val1, idx1)
        ...
        if sum(downMask==0)==length(downMask)
            idx2=[];
            val2=0;
            return;
        end
...
%         else
%             try 
%             kernelDiag = arrayfun(@(idx) kernelMat(data(idx, :), data(idx, :)), ...
%                 (1:nPoints)');
%             catch ME
%                 error('Error calculating the kernel function:\n%s\n',ME.message);
% %                 error('Bioinfo:svmtrain:KernelFunctionError','Error calculating the kernel function:\n%s\n',ME.message);
...
%         end
        ...
        function subKernelIndex = loadColumn(idx)
            ...
            % index 'kernelColIdx'.
            try
                subKernel(:,subKernelIndex) = kernelMat(:,idx);
            catch ME
                error('Error calculating the kernel function:\n%s\n',ME.message);
%                 error('Bioinfo:svmtrain:KernelFunctionError','Error calculating the kernel function:\n%s\n',ME.message);
