function [ftpr,ftnr,prec,gm,yPred,ftpr1,ftnr1,prec1,gm1,yPred1,ftpr2,ftnr2,prec2,gm2,yPred2] = ...
          KPboost_test(train_x,train_y,test_x,test_y,Dtr,sigma,lambda,bias,alpha,alpha1,alpha2,distTrTs)

T=size(Dtr,1);
Dts=ones(T,size(test_x,1));
...
for t=1:T
    if(t==1)
        K_new{t} = (Dtr(t,:)'*Dts(t,:)).*K_tr_ts;
    else
        K_new{t} = (Dtr(t,:)'*Dts(t,:)).*K_new{t-1}; 
    end
    [hyp_label(t,:)]=SVM_classify(test_x,train_x,train_y,lambda(:,t),bias(:,t),sigma,K_new{t});
%     posMask = (test_y==1); negMask = ~posMask;
%     out_posMask = (hyp_label(t,:)==1); out_negMask = ~out_posMask;
...
%     gmeans(t) = sqrt( (tp/(tp + fn)) * (tn/(tn + fp)) ); %Final Gmeans
%     fprintf('The testing G-means in iteration %d is: %d\n',t,gmeans(t));
